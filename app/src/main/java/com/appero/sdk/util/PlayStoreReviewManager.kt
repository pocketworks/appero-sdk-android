                ...
                when {
                    task.isSuccessful -> {
                        ...
                        ApperoLogger.logCriticalOperation(
                            "Play Store Review", "In-app review info obtained successfully"
                        )
                        ...
                        launchReviewFlow.addOnCompleteListener { flowTask ->
                            if (flowTask.isSuccessful) {
                                ...
                                ApperoLogger.logNetworkError(
                                    "Play Store Review",
                                    "In-app review flow failed: ${flowTask.exception?.message}"
                                )
                        ...
                        ApperoLogger.logNetworkError(
                            "Play Store Review",
                            "Failed to request review flow: $errorMessage"
                        )
    ...
    private fun fallbackToExternalStore(
        ...
        try {
            ...
            if (playStoreIntent.resolveActivity(activity.packageManager) != null) {
                ...
                ApperoLogger.logCriticalOperation(
                    "Play Store Review",
                    "Opened Play Store app successfully for package: $packageName"
                )
                ...
                ApperoLogger.logCriticalOperation(
                    "Play Store Review",
                    "Opened Play Store in web browser for package: $packageName"
                )
                onComplete?.invoke(ReviewResult.FallbackTriggered)
            }
