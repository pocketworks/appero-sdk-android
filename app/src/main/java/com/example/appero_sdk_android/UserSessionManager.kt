...
internal class UserSessionManager(private val sharedPreferences: SharedPreferences) {
    ...
    fun clearUserData(userId: String) {
        ...
        editor.remove(getUserKey(userId, USER_KEY_EXPERIENCE_POINTS))
        editor.remove(getUserKey(userId, USER_KEY_RATING_THRESHOLD))
        editor.remove(getUserKey(userId, USER_KEY_HAS_SUBMITTED_FEEDBACK))
